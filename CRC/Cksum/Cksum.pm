...
use 5.006;      # fairly arbitrary choice, actually.
use strict;
use warnings;
...
sub cksum($) {
    ...
    if(! ref $_[0]) {
        ...
        if(! defined $stat) {
            croak "error reading from handle: $!";
        }
    }

    my $nn = $n;        # save for the return
...
There is plenty of scope for a Cksum object
which can be fed data repeatedly
and then yields the sum when asked.
However, that will have to wait for some other 60 seconds...

=head1 DESCRIPTION

The String::CRC::Cksum module calculates a 32 bit CRC.
It generates the same CRC value as the POSIX cksum program.
...
Since I collected the algorithm from the Open Group web pages,
they might have some issues but I doubt it.
Let better legal minds than mine determine the issues if you need.
[hopefully the CPAN and PAUSE administrators will understand the issues
better, and will replace this entire section with something reasonable - hint hint.]
